            ...
            for d in ['/device:GPU:2', '/device:GPU:3']:
                with tf.device(d):
                    for batch in batches:
                        ...
                        train_step(cnn, x_batch, y_batch, history)
    ...
    #        if current_step % FLAGS.evaluate_every == 0:
            with tf.device('/cpu:0'):
                for test_batch in test_batches:
                    x_batch, y_batch = zip(*batch)
                    print("\nEvaluation:")
                    dev_step(cnn, x_dev, y_dev, history, writer=dev_summary_writer)
                    print("")


def train_step(cnn, x_batch, y_batch, history):
    """
...
def dev_step(cnn, x_batch, y_batch, history, writer=None):
    """"
